...
s32 stage_headers(spn_t* spn) {
  ...
  return render("/source/include/X11/XlibConf.h.in", "/store/include/X11/XlibConf.h", substs, 2);
}
...
spn_err_t configure(spn_t* spn, spn_config_t* config) {
  ...
  spn_node_add_output(headers, spn_get_subdir(spn, SPN_DIR_INCLUDE, "X11/XlibConf.h"));
  return SPN_OK;
}
