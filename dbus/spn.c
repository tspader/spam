...
s32 stage_headers(spn_t* spn) {
  if (spn_copy(spn, SPN_DIR_SOURCE, "dbus/*.h", SPN_DIR_WORK, "public/dbus")) {
    return 1;
  }
...
s32 package(spn_t* spn) {
  return spn_copy(spn, SPN_DIR_WORK, "public/dbus/*", SPN_DIR_INCLUDE, "dbus");
}
...
spn_err_t configure(spn_t* spn, spn_config_t* config) {
  ...
  spn_node_add_output(headers, spn_get_subdir(spn, SPN_DIR_WORK, "public/dbus/dbus.h"));
